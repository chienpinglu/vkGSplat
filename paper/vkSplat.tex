...
\title{vkSplat: A Compute-First Vulkan Path for\\Physical AI World Simulation and Synthetic Data Generation}
...
This does not require vkSplat to clone DLSS. DLSS is the precedent that
motivates moving reconstruction work onto compute, not a required
implementation dependency. The near-term vkSplat stack now prioritizes
...
\textbf{vkSplat} project. We make six claims:
...
\begin{enumerate}
    ...
    when the value-scalable fraction of a workload grows. In vkSplat,
...
compute backends coexist behind one stable contract. Concretely, vkSplat
aims to:
...
\begin{enumerate}
    ...
    frame resources. The compatibility burden belongs inside vkSplat,
...
story is just as important for vkSplat: \emph{graphics itself changed
...
\caption{The graphics model that vkSplat relies on. The upper row shows
...
central to vkSplat. If synthetic acquisition can be implemented as
...
the workload is priced in images per dollar or joule. vkSplat therefore
...
vkSplat does not need to clone DLSS. DLSS is the precedent that proves
the rendering value has already moved toward compute-side
reconstruction. The vkSplat question is narrower and more actionable:
...
answer plausible. vkSplat is the engineering experiment that turns that
...
vkSplat deliberately puts the bounded acquisition stages into software
...
fragment shading. The same organization is used for the 3DGS fast path:
...
denoising). The Vulkan-facing primitive expansion uses mesh shaders:
...
This is why vkSplat drops tessellation and geometry shaders from the
...
replication.  vkSplat needs a different question: how should a fixed
...
reason vkSplat treats fixed-function visibility as bounded acquisition
and keeps reconstruction on programmable compute.}
\label{fig:appendix-amdahl-threshold}
\end{figure}

The interpretation for vkSplat is direct.  Software visibility, shadow,
