...
A teaching-grade companion to **vkGSplat**: the same render job expressed
...
| `vulkan/`  | `tinycudarenderer_vulkan`   | Vulkan-shaped API → CUDA driver    | The vkGSplat thesis in microcosm.   |
...
- `vulkan/tinyvk.h` — a Vulkan-shaped C API (`Tvk*` types,
  ...
  `tvkCmdDrawMeshVKGSPLAT`, `tvkQueueSubmit`, …).
...
- `vulkan/main.cpp` — a Vulkan-flavoured application that knows
  ...
  hands it to the driver through `tvkCreateMeshVKGSPLAT`, records a
...
same. That is the entire vkGSplat pitch.

## Build

From the parent vkGSplat repo:
...
This produces, alongside the main vkGSplat targets:
...
`-DVKGSPLAT_BUILD_TINYCUDARENDERER=OFF` if you do not have a CUDA
toolchain.

If you would rather build it standalone (no parent vkGSplat, no
