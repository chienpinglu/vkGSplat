...
\title{vkGSplat: 3DGS Reconstruction for Vulkan-Compatible World Semantics and DLSS-Class Resolve}
\author{vkGSplat research note}
\date{Started May 5, 2026; updated May 12, 2026}
...
reconstruction state}: the reconstruction-side materialization of
Vulkan-compatible world semantics, not a standalone renderer or a bag of
unrelated tricks.
...
vkGSplat should keep software renderers as replaceable producers behind a
Vulkan-compatible acquisition contract. A CPU ray tracer, an Apple Metal
backend, a future CUDA software ray tracer, a \textsc{nvdiffrast}-style CUDA
triangle rasterizer, and a GraphDECO/\textsc{gsplat} Gaussian rasterizer should
all be allowed to produce the same seed-buffer contract. The 3DGS/DLSS-class
research layer begins after that boundary: it consumes color, depth, primitive
identity, motion, variance, and auxiliary signals, then performs history,
denoising, super-resolution, frame generation, and optional neural residual
reconstruction.
...
The data-center constraint is part of the technical problem. Omdia's Data
Center World / Analyst Summit 2026 deck frames AIDC growth as a trillion-dollar
infrastructure wave, with \$6.07T total IT spending in 2026, 72.9\% of surveyed
organizations expecting IT-budget increases, an upper data-center investment
scenario near \$1.9T by 2030, rack-scale AI systems, 800VDC power distribution,
BESS, self-generation, and liquid cooling moving into the mainstream
\cite{omdia2026dcw}. For this note, those numbers do not prove that a Gaussian
reconstructor wins. They define the operating constraint: the reconstruction
state must look like a normal AI-data-center tensor workload, not a bespoke
graphics island that forces separate scheduling, storage, cooling, profiling,
...
3DGS is useful here only if it satisfies five criteria:
...
\texttt{papers/vkGSplat.tex}. The system paper defines the application-facing
contract: capture existing Vulkan/Wicked workloads, lower acquisition to
CUDA-class compute, and preserve a stable seed-buffer ABI. This note
deliberately starts after that boundary. It asks whether a persistent Gaussian
state is the right reconstruction and reuse layer once seed evidence already
exists. In the master-paper terminology, the seed ABI is the
Vulkan-compatible acquisition contract, while the persistent Gaussian cache is a
concrete implementation of Vulkan-compatible world semantics. Therefore the two
papers should cross-reference each other rather than duplicate material: the
system paper owns the Vulkan/SDG/hardware-allocation argument, while this note
owns the 3DGS reconstruction, tensorization, radiance-cache, and
...
\downarrow\\
\text{software acquisition renderer behind a Vulkan-compatible contract}\\
\downarrow\\
\text{seed buffers: color, depth, IDs, motion, variance, metadata}\\
\downarrow\\
\text{Vulkan-compatible world semantics in persistent Gaussian state}\\
\downarrow\\
\text{3DGS/DLSS-class reconstruction and resolve}
\end{gathered}
\]
...
interpolation, Gaussian state maintenance, and any learned residual. It is not
a new public IR and not a Vulkan extension. It is a private semantic cache that
records the world facts needed by CUDA/PyTorch reconstruction while preserving
ordinary Vulkan-facing application behavior. It should not branch on whether
the seed came from CPU, Metal, or CUDA acquisition.
...
producers of the same Vulkan-compatible seed ABI.
...
\section{AIDC Constraint on the Reconstruction Layer}

The main vkGSplat paper makes the economic argument: SDG should be able to run
as an ordinary workload on the AI data center when that is where the capital,
power, cooling, scheduling, and model-training infrastructure already exists.
This note turns that argument into reconstruction-kernel constraints. The
Gaussian state is useful only if it is a \emph{resident tensor state}: compact
enough to stay near the compute, structured enough for PyTorch/CUDA extension
interop, and reusable enough that TAA, super-resolution, denoising, frame
generation, radiance caching, and neural resolving do not each build their own
history representation.

\begin{center}
\begin{tabular}{p{0.31\linewidth}p{0.58\linewidth}}
\toprule
Omdia/AIDC signal & Reconstruction consequence for vkGSplat \\
\midrule
Rack-scale AI systems and high-power AI clusters & Keep acquisition, Gaussian
update, reconstruction, and training in one CUDA/PyTorch residency domain. \\
800VDC, BESS, self-generation, and liquid-cooling pressure & Optimize
joules/useful-sample and HBM bytes, not only per-frame latency. \\
Token demand rising from generative, agentic, and Physical AI workloads & Treat
SDG as a batch/streaming tensor workload that can fill AI-cluster capacity. \\
AI-capex scale and operational lock-in & Avoid designs requiring a separate
graphics-only fleet unless they clearly win on end-to-end cost. \\
\bottomrule
\end{tabular}
\end{center}

The direct kernel implication is that the shared Gaussian state must expose
AIDC-style counters in every experiment: device-resident bytes, HBM traffic,
kernel launches, tensor-core utilization where relevant, sort/prefix-scan cost,
host-device copies, interop copies, energy per reconstructed output, and reuse
count per Gaussian page or tile list. If the unified state improves PSNR or
temporal stability but forces extra data movement across a graphics/AI boundary,
it has failed the system-level vkGSplat argument even if it looks good in an
isolated renderer.
...
This tensor ABI is the concrete form of Vulkan-compatible world semantics for
the reconstruction paper. The public side remains ordinary Vulkan objects and
command streams; the private CUDA side sees dense seed tensors, sparse Gaussian
state, tile lists, radiance pages, confidence, variance, and provenance. That
split is the main reason the 3DGS layer can be acquisition-independent while
still staying useful to existing engines.
...
The current M0 CUDA code implements this ABI as a deliberately small tensor
pipeline:

\begin{center}
\begin{tabular}{llll}
\toprule
Stage & CUDA entry point & Tensor shape & Purpose \\
\midrule
Extract & \texttt{launch\_extract\_nvdiffrast\_sample\_tensors} & $BHW\rightarrow M_s$ & compact valid raster/interpolate hits \\
Counted update & \texttt{launch\_accumulate\_...\_counted} & $M_s\rightarrow N$ & consume device-side sample count without host sync \\
Project & \texttt{launch\_project\_gaussian\_state\_features} & $N\rightarrow HW$ & emit feature planes; optional \texttt{motion\_alpha} evaluates 4DGS-lite time \\
Tile count & \texttt{launch\_build\_gaussian\_sample\_tile\_bins} & $M_s\rightarrow T$ & produce per-sample tile IDs and per-tile counts \\
Tile compact & \texttt{launch\_compact\_gaussian\_sample\_tile\_bins} & $(M_s,T)\rightarrow M_s$ & scatter compact sample indices into tile-local spans \\
Tile resolve & \texttt{launch\_resolve\_gaussian\_sample\_tiles} & $(T,M_s)\rightarrow HW$ & consume tile spans and emit radiance/depth/id feature planes \\
Weighted resolve & \texttt{launch\_resolve\_...\_weighted} & $(T,M_s,r^2)\rightarrow HW$ & accumulate Gaussian footprint taps, then normalize radiance/depth/confidence \\
Gated resolve & \texttt{launch\_resolve\_...\_weighted\_gated} & $(T,M_s,r^2,G)\rightarrow HW$ & reject taps by primitive/depth/normal/motion guide tensors before accumulation \\
\bottomrule
\end{tabular}
\end{center}

The tile-list contract is intentionally compatible with both CUB-style scans
and PyTorch experimentation. For a flattened tile grid of $T$ tiles, define
$c_t$ as the value stored in \texttt{tile\_counts} for tile $t$. The exclusive
offset vector $O\in \mathbb{Z}^{T+1}$ is
\[
  O_0=0,\qquad O_{t+1}=O_t+c_t .
\]
\texttt{launch\_compact\_gaussian\_sample\_tile\_bins} then writes sample
indices into
\[
  \texttt{tile\_sample\_indices}[O_t, O_{t+1}) .
\]
This is the first reusable primitive for tensorized TAA/SR/frame-generation
kernels: later projection and resolve kernels can launch block-per-tile work
and touch only local sample or Gaussian members. The current direct
\texttt{launch\_resolve\_gaussian\_sample\_tiles} kernel is a plumbing oracle:
one compact sample maps to one output pixel. The companion
\texttt{launch\_resolve\_gaussian\_sample\_tiles\_weighted} path is the first
reconstruction-shaped CUDA kernel: for footprint radius $r$, it emits
confidence-weighted Gaussian taps into radiance and depth accumulators using
atomic adds, then normalizes a dense $HW$ tensor. Its initial $r=0$ mode should
match the direct resolver exactly; $r>0$ is the first place to measure blur,
ghosting, variance reduction, and atomics cost. The M1
\texttt{launch\_resolve\_gaussian\_sample\_tiles\_weighted\_gated} variant adds
dense guide tensors $G$: primitive IDs, depth/confidence, optional normals, and
optional motion. A tap is rejected before accumulation when it crosses a
primitive boundary, exceeds the depth epsilon, fails the normal dot threshold, or
exceeds the motion residual threshold. Its counters are the first CUDA-side
ghosting diagnostics: emitted taps versus primitive/depth/normal/motion
rejections. Later variants should add visibility ordering and neural or analytic
residual channels. The present binding leaves the exclusive scan as a tensor
operation, e.g. \texttt{torch.cumsum} in experiments or CUB/Thrust in
production, so the custom CUDA code stays focused on the irregular count,
scatter, and tile-local resolve parts.
...
\subsection{4DGS-Lite for TAA}

The TAA path should use ideas from 4DGS, but the first CUDA prototype should
not require a fully learned dynamic 4DGS model. The useful 4DGS idea is that
history is a continuous-time Gaussian object, not only an image-space previous
color. Represent the motion-critical part of each Gaussian as
\[
  G_j(t)=
  \{\mu_j(t),\Sigma_j(t),\bar L_j(t),\alpha_j(t),\nu_j(t)\},
\]
where $\nu_j(t)$ is temporal confidence. The M1 implementation can start with
constant-velocity motion,
\[
  \mu_j(t)=\mu_j(t_0)+v_j(t-t_0),
  \qquad
  \Sigma_j(t)=\Sigma_j(t_0)+\sigma^2_{v,j}(t-t_0)^2 I,
\]
so motion uncertainty widens the projected footprint rather than pretending the
old sample is exact. The TAA resolve then evaluates and projects $G_j(t)$ at the
current frame time and current jitter phase. This is the 4DGS counterpart of
image-space reprojection: the previous observation is carried by a moving
Gaussian state before it is sampled in screen space.

Full learned 4DGS or deformation-basis methods are reserved for later stages.
They become necessary only if the analytic state fails on nonrigid motion,
rolling shutter, topology changes, or acceleration. The first pass/fail test is
therefore: can the analytic 4DGS-lite state reduce shimmer and ghosting against
image-space TAA while sharing the same tile lists used by SR and frame
interpolation? If yes, 4DGS is the right abstraction; if no, learned dynamic
4DGS features should be tested as an explicit ablation rather than assumed.
...
\subsection{M0R: Minimal CUDA Render Bridge}

The current code should also keep a deliberately narrow CUDA render bridge in
place, because reconstruction experiments need an end-to-end projected-splat
path even before the full CUDA rasterizer is finished. This bridge is not the
final renderer. It is an implementation scaffold with one important property:
the device tile renderer consumes the same \texttt{GpuProjectedSplat},
\texttt{GpuTileRange}, and sorted-index ABI that later TAA, SR, interpolation,
and feature-projection kernels should reuse.

The present M0R split is intentionally simple:

\begin{itemize}
  \item projection, covariance footprint, opacity, SH/color evaluation, and
        per-Gaussian tile coverage now run in a CUDA preprocessor,
  \item the renderer now builds deterministic device-side tile lists and
        \texttt{GpuTileRange} records with a fixed per-tile capacity, avoiding
        host readback in the M0 render path,
  \item fixed-list overflow is counted on device and reported after stream
        synchronization, so dense scenes expose capacity loss instead of
        silently dropping splats,
  \item this fixed-list kernel is intentionally a bridge: it proves the ABI and
        deterministic range construction before the production count/scan/
        scatter and depth-key sort path replaces it,
  \item alpha compositing already runs through CUDA \texttt{TileBlend}, which
        clears the output, clamps opacity, and stops once transmittance is
        small,
  \item \texttt{RenderTargetKind::INTEROP\_IMAGE} is now part of the bridge:
        the tile renderer can pack RGBA8 and write directly to a CUDA surface
        object, including the surface object produced by a Vulkan external
        image import. Host-buffer readback remains a test/debug path, not the
        intended residency boundary.
\end{itemize}

This is acceptable only as a short bridge. The first pass/fail test is whether
the CUDA tile blend matches the CPU 3DGS reference for tiny scenes after both
are given identical projected splats and tile ranges. The second test replaces
one host-owned stage at a time and checks that image error, surface output, and
tile statistics remain stable. The replacement order should be:

\begin{enumerate}
  \item validate that \texttt{HOST\_BUFFER} RGBA8 output and
        \texttt{INTEROP\_IMAGE} CUDA-surface output agree on the same tiny
        projected-splat fixture,
  \item replace the fixed-capacity tile-list kernel with device tile-count
        emission, exclusive scan, and compact scatter,
  \item add depth-key generation and either CUB radix sort or bounded tile-local
        sort,
  \item reuse the same tile ranges for TAA, SR, feature projection, and
        frame-generation kernels instead of building renderer-only ranges.
\end{enumerate}

M0R fails if the renderer builds a private tile-list ABI that the
reconstruction kernels cannot consume. It succeeds if render, TAA, SR, and
frame interpolation all use the same projected-splat/range contract, even while
some construction stages are still host oracles. The key implementation lesson
is that software rendering addresses the fixed-function graphics dependency
only if its output is already device-resident; the 3DGS reconstruction stack
addresses the DLSS boundary only if that device-resident output is immediately
visible as Gaussian/radiance history rather than a final image copied back to
the CPU.
...
public:
    ...
    void build_sample_tile_bins_cuda(const SampleTensorDeviceView& samples);
    void compact_sample_tile_bins_cuda(const TileOffsetDeviceView& offsets);
    void resolve_sample_tiles_weighted_cuda(const WeightedResolveParams& params);
    void resolve_sample_tiles_weighted_gated_cuda(const GatedResolveParams& params);
...
\begin{itemize}
  ...
  \item host-device copies, Vulkan/CUDA interop copies, and cross-process or
        cross-cluster transfers,
  \item energy per useful reconstructed output, not only energy per raw frame.
\end{itemize}

\subsection{AIDC Residency Metrics}

The Omdia-driven system constraint is that vkGSplat should behave like an
AI-data-center workload. Each benchmark should therefore report a residency
score in addition to visual quality:

\[
  R_{\mathrm{aidc}} =
  1 -
  \frac{
    B_{\mathrm{host}} + B_{\mathrm{interop}} + B_{\mathrm{remote}}
  }{
    B_{\mathrm{device}} + B_{\mathrm{host}} + B_{\mathrm{interop}}
    + B_{\mathrm{remote}} + \epsilon
  },
\]
where $B_{\mathrm{device}}$ is useful device-resident traffic inside the CUDA
pipeline, $B_{\mathrm{host}}$ is host readback/writeback, $B_{\mathrm{interop}}$
is graphics/compute interop copying, and $B_{\mathrm{remote}}$ is data moved to
another storage or cluster boundary. A successful vkGSplat reconstruction path
should keep $R_{\mathrm{aidc}}$ close to one while improving quality or reuse.
This metric makes the economic claim measurable: a visually strong
reconstructor that exports large intermediate images to a separate graphics or
storage path is not AIDC-native.
...
\begin{enumerate}
  ...
  \item \textbf{Shared core plus AIDC residency constraint:} same as variant 3
        or 4, but disallow host readback and cross-cluster intermediate images;
        all seed tensors, Gaussian pages, radiance pages, and training batches
        must remain CUDA/PyTorch resident until final dataset export.
\end{enumerate}
...
validation time and measuring the error jump. Variant 5 is the system-level
gate: if it loses most of the quality or throughput gain, the reconstruction
idea may be interesting but it does not support the main vkGSplat economic
argument.
...
The near-term wording should stay disciplined. Do not present the Gaussian
state as a general scene IR. Present it as Vulkan-compatible world semantics:
the private CUDA/PyTorch reconstruction state inferred from a
Vulkan-compatible acquisition contract. That makes the claim concrete enough to
test in the current repository, while still leaving room for future sensor and
Physical AI semantics.
...
\section{Appendix: Physical AI Accuracy and Sensor Modeling}

A vkGSplat renderer should not claim physical accuracy merely because it stores
explicit 3D/4D Gaussian state. A Gaussian state is an excellent carrier for
persistent measured evidence, uncertainty, visibility, motion, and radiance. It
is not, by itself, a physics simulator or a calibrated sensor simulator. This
matters for Physical AI, where synthetic data and closed-loop simulation are
used for robotics, autonomous vehicles, and industrial agents. NVIDIA describes
Cosmos as an open platform for Physical AI with world foundation models, and
Isaac Sim as a robotics simulator for physically based virtual environments,
robotics testing, and synthetic data generation \cite{nvidia2026cosmos,
nvidia2026isaacsim}. Isaac Sim also separates rendering from physics-based
sensors: those sensors run after rendering, access physics properties such as
mass and velocity, and emit physics-engine measurements that may be augmented by
post-process noise \cite{nvidia2026isaacphysicssensors}. vkGSplat should follow
the same separation of authority: Vulkan-compatible world semantics preserves
and organizes acquired evidence, but it does not become the physical oracle.

The most important design rule is that physical realism enters primarily during
acquisition. In a real deployment, acquisition means calibrated cameras, LiDAR,
radar, IMU, joint, contact, and timing systems. In simulation, acquisition means
a physics engine plus calibrated raster/ray/path/sensor models. vkGSplat can
cache, fuse, denoise, replay, and temporally reconstruct this evidence, but it
cannot recover missing physics that was never acquired or simulated. Once a
specular path, rolling-shutter timing error, contact impulse, LiDAR dropout, or
radar multipath event is absent from the seed data, a Gaussian resolver can only
hallucinate a plausible substitute.

The correct claim is therefore:

\begin{quote}
vkGSplat can contribute Physical AI-grade sensor realism as a fast,
uncertainty-aware reconstruction and sensor-cache layer, but the authoritative
physical state must come from the acquisition phase: real calibrated sensors,
physics simulation, calibrated ray/sensor equations, and validation against real
or high-fidelity reference data.
\end{quote}

Let $S_t$ be the simulator state: rigid and articulated poses, velocities,
contact state, materials, lights, and environment state. Let $R_t$ be sparse or
expensive rendering evidence from rasterization, ray tracing, path tracing, or a
reference sensor simulator. Let $\mathcal G_t$ be the persistent Gaussian state.
A safe Physical AI formulation is

\[
  \mathcal G_t = U_\theta(\mathcal G_{t-1}, S_t, R_t, A_t),
\]

where $A_t$ contains raster or sensor seed buffers: depth, primitive identity,
world position, normal, motion, material IDs, confidence, and variance. For each
sensor $s$, vkGSplat predicts a measurement

\[
  \hat y_{s,t} = H_s(\mathcal G_t, S_t, C_s) + \epsilon_s,
\]

where $C_s$ is calibration and $\epsilon_s$ is a calibrated noise model. The
important discipline is that $H_s$ is sensor-specific. There is no generic
``render RGB and call it a sensor'' shortcut for Physical AI. RGB, depth, LiDAR,
radar, event cameras, IMU, joint effort, and contact sensors have different
measurement equations, failure modes, rates, and timing.

\begin{center}
\small
\begin{tabular}{p{0.20\linewidth}p{0.34\linewidth}p{0.34\linewidth}}
\toprule
Layer & Owns & vkGSplat role \\
\midrule
Physics engine & dynamics, contact, mass, friction, joint state, IMU/contact/joint truth & consume state and timestamps; do not infer these from Gaussians \\
Raster/ray/path oracle & visibility, occlusion, high-quality radiance, range hits, rare paths & provide sparse correction samples and confidence gates \\
Persistent Gaussian state & measured geometry/radiance/features, variance, visibility, motion, material IDs & fast cache and reconstruction substrate \\
Sensor model & calibration, optics, scan timing, noise, dropout, quantization, rolling shutter & project Gaussian/physics state into calibrated measurements \\
Neural resolver & learned priors for image/video completion & optional; must be uncertainty-gated and validated \\
\bottomrule
\end{tabular}
\end{center}

This makes the framework physically useful without pretending that 3DGS is the
physical oracle. The Gaussian state can store metric position, covariance,
normal, opacity, radiance, BRDF/material features, semantic and primitive IDs,
velocity, angular velocity or deformation parameters, timestamp, confidence,
variance, visibility age, and sensor-specific residuals. CUDA kernels can then
query this state at high rate. However, if confidence is low, if disocclusion is
large, if material/path behavior is outside the Gaussian model, or if a safety
critical sensor is being evaluated, the framework must fall back to the
authoritative physics/ray/sensor path.

\begin{center}
\scriptsize
\begin{tabular}{p{0.12\linewidth}p{0.34\linewidth}p{0.36\linewidth}p{0.10\linewidth}}
\toprule
Sensor & Required model & Gaussian contribution & Risk \\
\midrule
RGB camera & lens distortion, aperture/PSF, exposure, rolling shutter, motion blur, Bayer/ISP, shot/read/dark noise, tone map/compression & fast radiance and feature reconstruction, temporal history, uncertainty map & plausible but radiometrically wrong pixels \\
Depth/stereo/ToF & baselines, occlusion, IR response, multipath, quantization, flying pixels, confidence & dense depth prior and disocclusion masks & wrong edges and reflective/transparent failures \\
LiDAR & beam divergence, scan timing, range noise, incidence, intensity, dropout, multi-return, weather & approximate range/reflectance cache; fast local replay & missed thin objects and wrong multipath \\
Radar & Doppler, antenna pattern, wave scattering, material response, multipath & learned or cached surrogate only & weak physical fit; needs ray/wave oracle \\
Event camera & log-intensity derivative, threshold variation, refractory period, timestamp jitter & continuous-time radiance/motion state & false events under interpolation errors \\
IMU/contact/joint & accelerations, angular velocity, forces, torques, contact impulses from physics & synchronization and cross-sensor consistency only & should not be generated from GS \\
\bottomrule
\end{tabular}
\end{center}

For CUDA implementation this implies a sensor-facing extension of the current
M0 state. The base structure should remain SoA, but add optional pages:

\[
  G_i = (\mu_i, \Sigma_i, q_i, c_i, \alpha_i, f_i, m_i, v_i, \omega_i,
         id_i, \tau_i, \nu_i, \sigma_i^2, r_{i,s}).
\]

Here $q_i$ is orientation, $c_i$ radiance or spectral color, $f_i$ learned or
analytic features, $m_i$ material/sensor reflectance parameters, $v_i$ and
$\omega_i$ motion, $\tau_i$ timestamp, $\nu_i$ confidence, $\sigma_i^2$
variance, and $r_{i,s}$ optional residuals for sensor $s$. A camera projection
kernel uses $\mu_i,\Sigma_i,c_i,v_i,\tau_i$ and calibration. A LiDAR kernel uses
ray/beam parameters, opacity or density, surface normal, material reflectance,
and range-confidence gates. A physics sensor kernel should normally not query
$G_i$ except for synchronization or cross-sensor consistency; its measurement
comes from $S_t$.

The current implementation exposes this as a small host-side contract rather
than a monolithic simulator. The \texttt{sensor\_model} module provides a
\texttt{CameraSensorModel} for calibrated exposure, noise, quantization,
vignetting, and rolling-shutter timing, plus a
\texttt{SensorPreservationPolicy} gate that decides whether a reconstructed
Gaussian observation is safe to reuse or whether the system must request the
physics/ray/sensor oracle. This keeps the CUDA reconstruction path compatible
with Physical AI while preserving the central rule: learned 3DGS state can carry
measured realism, but it does not create missing physical evidence.

The pass/fail bar should be statistical and task-level, not only image-quality
metrics:

\begin{enumerate}
  \item \textbf{Camera/radiance:} compare against path-traced or real-calibrated
        data using linear radiance error, PSNR/SSIM/LPIPS, temporal flicker,
        depth/normal error, and exposure/ISP ablations.
  \item \textbf{Depth/LiDAR:} measure range error by distance, incidence angle,
        material, edge proximity, and weather; report dropout, multi-return, and
        false-positive statistics.
  \item \textbf{Perception transfer:} train or evaluate a detector/segmenter/
        depth estimator on vkGSplat sensor output and compare against real-log
        or high-fidelity simulator performance.
  \item \textbf{Closed-loop control:} run a robot or vehicle policy using exact
        simulated sensors, vkGSplat sensors, and vkGSplat plus oracle fallback;
        compare success, collisions, interventions, and trajectory drift.
  \item \textbf{Safety stress:} evaluate glass, mirrors, thin geometry, rain,
        dust, low light, high speed, rolling shutter, disocclusions, and dynamic
        occluders. These should be explicit failure tests.
\end{enumerate}

The experiment that decides whether vkGSplat belongs in a Physical AI stack is
an ablation with equal compute and bandwidth budgets:

\begin{center}
\small
\begin{tabular}{p{0.22\linewidth}p{0.32\linewidth}p{0.34\linewidth}}
\toprule
System & Expected strength & Deciding metric \\
\midrule
Exact physics/ray sensors & best physical accuracy, slowest & reference quality and safety baseline \\
Image-space neural reconstruction & fast and visually plausible & domain gap and temporal/sensor failures \\
Separate per-sensor GS caches & better sensor specialization & memory cost and cross-sensor consistency \\
Shared vkGSplat state & shared geometry/motion/visibility evidence & accuracy per watt and task transfer \\
Shared state plus oracle fallback & best safety profile & failure rate under stress scenes \\
\bottomrule
\end{tabular}
\end{center}

If the shared Gaussian state plus fallback matches the task-level behavior of
exact sensors while saving substantial time or energy, vkGSplat is a strong
Physical AI sensor substrate. If it only improves RGB appearance while harming
range, timing, contact, or policy behavior, then it should remain a visual
reconstruction tool rather than a Physical AI simulator.
...
\bibitem{nvidia2026cosmos}
NVIDIA.
\newblock NVIDIA Cosmos: Physical AI with World Foundation Models.
\newblock Accessed May 5, 2026.
\newblock \url{https://www.nvidia.com/en-us/ai/cosmos/}

\bibitem{nvidia2026isaacsim}
NVIDIA Developer.
\newblock NVIDIA Isaac Sim: Robotics Simulation and Synthetic Data Generation.
\newblock Accessed May 5, 2026.
\newblock \url{https://developer.nvidia.com/isaac-sim}

\bibitem{nvidia2026isaacphysicssensors}
NVIDIA.
\newblock Isaac Sim Documentation: Physics-Based Sensors.
\newblock Accessed May 5, 2026.
\newblock \url{https://docs.isaacsim.omniverse.nvidia.com/6.0.0/sensors/isaacsim_sensors_physics.html}
...
\bibitem{omdia2026dcw}
Omdia.
\newblock Data Center World / Omdia Analyst Summit 2026: The Trillion Dollar
AIDC Boom.
\newblock Presentation deck, April 2026. Local source file:
\texttt{ai\_progress/external/DCW OAS 2026.pdf}.
